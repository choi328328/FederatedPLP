...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 4, layers[0]) # was 64
        self.layer2 = self._make_layer(
            block, 4, layers[1], stride=2, dilate=replace_stride_with_dilation[0]
        ) # was 128
        self.layer3 = self._make_layer(
            block, 8, layers[2], stride=2, dilate=replace_stride_with_dilation[1]
        ) # was 256
        self.layer4 = self._make_layer(
            block, 16, layers[3], stride=2, dilate=replace_stride_with_dilation[2]
        ) # was 512
        self.avgpool = nn.AdaptiveAvgPool1d((1))
        self.fc = nn.Linear(16 * block.expansion, num_classes) # was 512
...
def resnet_simple(pretrained=False, progress=True, **kwargs):
    return _resnet(
        "resnet_simple", BasicBlock, [1, 1, 2, 1], pretrained, progress, **kwargs
    )

def resnet_simplest(pretrained=False, progress=True, **kwargs):
    return _resnet(
        "resnet_simplest", BasicBlock, [1, 1, 1, 1], pretrained, progress, **kwargs
    )
