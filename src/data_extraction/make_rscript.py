...
def renew_run_script(client_conf):
    cfg=client_conf
    run_script= f'''
    library({cfg.package})
    # USER INPUTS
    ...
    outputFolder <- "./{cfg.project}Results"
    ...
    predictionAnalysisListFile <- system.file("settings",
                                            ...
                                            package = "BipolarMisclassificationV2")
    ...
    '''+f'''
    arrow::write_parquet(pop_df, sink = file.path('./temps/', "cov_{cfg.project}.parquet"))
    '''
    return run_script